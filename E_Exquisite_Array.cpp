...
int main()
{
    ...
    while (t--)
    {
        ...
        vector<ll> vec(n);
        cin >> vec;
        vector<vector<ll>> poses(n);
        for (ll i = 0; i < n - 1; i++)
        {
            ll diff = abs(vec[i + 1] - vec[i]);
            poses[diff].push_back(i);
        }
        set<pll> stt;
        stt.insert({0, n - 1});
        ...
        for (ll i = 1; i <= n - 1; i++)
        {
            ...
            for (auto it : poses[i])
            {
                auto pos = stt.upper_bound({it, LLONG_MAX});
                pos--;
                pll now = *pos;
                ll l = now.first;
                ll r = now.second;
                ...
                if (l <= it && r >= it + 1)
                {
                    ...
                    stt.erase(pos);
                    if (now.first < it)
                        stt.insert({now.first, it});
                    if (it + 1 < now.second)
                        stt.insert({it + 1, now.second});
                }
            }
        }
        cout << fans << nn;
