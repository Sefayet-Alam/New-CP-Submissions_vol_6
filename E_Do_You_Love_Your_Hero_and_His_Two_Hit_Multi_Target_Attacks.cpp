...
int main()
{
    ...
    while (t--)
    {
        ...
        while (k>0)
        {
            ...
            auto pos=upper_bound(all(sqs),k);
            pos--;
            ll x=pos-sqs.begin();
            k-=(*pos);
            xs.push_back(x);
        ...
        ll kcnt=0;
        for(ll i=0;i<sz;i++){
            ll xa=ans[i].first,ya=ans[i].second;
            for(ll j=i+1;j<sz;j++){
                ll xb=ans[j].first,yb=ans[j].second;
                ll lft=abs(xa-xb)+abs(ya-yb);
                ll rg=sqrt((xa-xb)*(xa-xb)+(ya-yb)*(ya-yb));
                if(lft==rg) kcnt++;
            }
        }
        // deb2(kcnt,k2);
