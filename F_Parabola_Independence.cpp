...
int main()
{
    ...
    while (t--)
    {
        ...
        sort(all(vec), cmp);
        vector<ll> dpen(n, 1), dpst(n, 1);
        for (ll i = 0; i < n; i++)
        {
            for (ll j = 0; j < i; j++)
            {
                if (isless(vec[j], vec[i]))
                {
                    dpen[i] = max(dpen[i], dpen[j] + 1);
                }
            }
        }
        for (ll i = n - 1; i >= 0; i--)
        {
            for (ll j = i + 1; j < n; j++)
            {
                if (isless(vec[i], vec[j]))
                {
                    dpst[i] = max(dpst[i], dpst[j] + 1);
                }
            }
        }
        vector<ll> ans(n);
