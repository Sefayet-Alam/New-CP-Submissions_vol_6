...
int main()
{
    ...
    while (t--)
    {
        ...
        ll pathcells = n + m - 1;
        vl hmax1(pathcells,-M);
        vl hmax2(pathcells,-M);
        vl cnt(pathcells,0);

        for(ll i=0;i<n;i++){
            for(ll j=0;j<m;j++){
                ll k=i+j;
                ll hval=f[i][j]+g[i][j]-a[i][j];
                if(hval>hmax1[k]){
                    hmax2[k]=hmax1[k];
                    hmax1[k]=hval;
                    cnt[k]=1;
                }
                else if(hval==hmax1[k]){
                    cnt[k]++;
                }
                else if(hval>hmax2[k]){
                    hmax2[k]=hval;
                }
            }
        }
        ...
        for(ll i=0;i<n;i++){
            for(ll j=0;j<m;j++){
                ll k=i+j;
                ll nowhval=f[i][j]+g[i][j]-a[i][j];
                ll avoid=(nowhval==hmax1[k] && cnt[k]==1)?hmax2[k]:hmax1[k];
                ll ultay=nowhval-2*a[i][j];
                ll nowbest=max(avoid,ultay);
                ans=min(ans,nowbest);
            }
        }
        cout<<ans<<nn;
