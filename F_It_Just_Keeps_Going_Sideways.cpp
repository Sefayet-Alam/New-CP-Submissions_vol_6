...
int main()
{
    ...
    while (t--)
    {
        ...
        b=vec;
        sort(all(b));
        vector<ll>pos(n+1,0),pos2(n+1,0);
        ...
        sg2.build(pos2);
        for(ll i=0;i<n;i++){
            sg1.RangeUpdate(0,vec[i]+1,i);
            sg2.RangeUpdate(0,b[i]+1,i);
        }
        for(ll i=0;i<=n;i++){
            pos[i]=sg1.RangeQuery(i,i+1);
            pos2[i]=sg2.RangeQuery(i,i+1);
        }
        ll ans=0;
        for(ll i=0;i<=n;i++){
        ...
        ll maxm=0;
        vector<ll>pos3(n+1,0);
        segment_tree sg3;
        sg3.init(n+1);
        sg3.build(pos3);
        for(ll i=0;i<n;i++){
            sg3.RangeUpdate(1,vec[i]+1,1);
        }
        for(ll i=n-1;i>=0;i--){
            sg3.RangeUpdate(1,vec[i]+1,-1);
            ll x=sg3.RangeQuery(vec[i],vec[i]+1);
            // deb2(i,x);
            maxm=max(maxm,x);
        }
        cout<<ans+maxm<<nn;
